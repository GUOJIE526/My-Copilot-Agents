...
- If the user invokes an OpenSpec proposal command such as `/opsx-ff` or `/opsx-propose`, do not stop after proposal generation. Automatically continue the full workflow in sequence: complete the proposal, then run `/opsx-apply` for implementation, `/opsx-verify` for validation, and `/opsx-archive` for archival without waiting for a separate user prompt.
...
- **Before starting implementation, you MUST use the prompt itself or an OpenSpec proposal request to analyze which rules under `.github/instructions` apply to the work, study those instructions first**, and only then proceed with development.
- When working on `Admin` or `AdminAngular` projects, you MUST follow the relevant admin instruction files under `.github/instructions`; when working on the `Api` project, you MUST follow the relevant api instruction files under `.github/instructions`.
