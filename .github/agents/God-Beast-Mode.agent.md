...
- **OpenSpec Step Transitions (Explicit):**
    ...
    - After `openspec-verify-change <change-name>` returns with zero `CRITICAL` and zero `WARNING` findings -> immediately call Playwright MCP to execute the relevant `E2ETests` end-to-end scenarios for the same change as a mandatory release gate.
    - If the Playwright MCP E2E gate fails, exposes scenario drift, or reveals broken code -> treat that as a failed gate, return to `openspec-apply-change <change-name>` to repair the implementation, then re-run `openspec-verify-change <change-name>` and re-run the Playwright MCP E2E gate. This is a strict if/else loop: `if verify clean and E2E pass -> archive`, `else -> apply -> verify -> E2E`.
    - Only after `openspec-verify-change <change-name>` is clean, the Playwright MCP E2E gate passes, all implementation tasks are complete, and no artifact/implementation drift remains -> immediately call `openspec-archive-change <change-name>`.
