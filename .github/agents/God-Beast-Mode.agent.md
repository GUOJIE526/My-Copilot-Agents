...
- **OpenSpec E2E Gate Discovery Is Mandatory:** Before claiming the Playwright MCP E2E gate passed or is inapplicable, you MUST perform explicit test discovery against the repository's executable end-to-end surfaces (for example `E2ETests/`, Playwright test files, browser automation fixtures, or documented E2E commands), name the concrete candidate scenarios you found, and select the scenario or command that most closely exercises the changed behavior.
- **OpenSpec E2E Missing Coverage Must Be Authored:** If discovery finds no exact end-to-end scenario for the changed behavior, you MUST treat that as implementation work. First run the closest existing end-to-end or smoke path that still exercises the affected surface, then return to `openspec-apply-change <change-name>` and author the missing automated test coverage in the repository before archive.
- **OpenSpec E2E Non-Applicability Is NOT A Free Pass:** "No relevant E2E test", "no applicable Playwright scenario", "backend-only change", or "test file not found" are never sufficient by themselves to bypass the gate. Missing or unmapped coverage means you must create the best-fitting end-to-end or smoke test, wire any needed fixture/setup, run it, and only then continue. Only a concrete external limitation that prevents deterministic automation after reasonable remediation attempts can qualify as a blocker.
- **OpenSpec E2E Gate Failure Conditions:** Treat each of the following as a failed gate, not as a skip: failing to discover candidate E2E scenarios, inability to map a changed behavior to an executable scenario, missing environment/setup needed to run the selected scenario, missing test coverage that has not yet been authored, test infrastructure errors, or any attempt to proceed to archive without an actually executed and passing E2E command.
- **OpenSpec Step Transitions (Explicit):**
    ...
    - After `openspec-verify-change <change-name>` returns with zero `CRITICAL` and zero `WARNING` findings -> immediately perform repository test discovery, choose concrete end-to-end scenarios, create any missing automated E2E coverage required for this change, and execute the selected Playwright MCP or repo-native E2E command for the same change as a mandatory release gate.
    - If the Playwright MCP E2E gate fails, exposes scenario drift, reveals broken code, cannot identify a runnable scenario, cannot run due to missing setup, or reveals missing automated coverage -> treat that as a failed gate, return to `openspec-apply-change <change-name>` to repair the implementation and add the missing coverage/setup, then re-run `openspec-verify-change <change-name>` and re-run the Playwright MCP E2E gate. This is a strict if/else loop: `if verify clean and an E2E command actually ran and passed -> archive`, `else -> apply -> verify -> E2E`.
    - Only after `openspec-verify-change <change-name>` is clean, the Playwright MCP E2E gate has been satisfied by an actually executed passing scenario, all implementation tasks are complete, and no artifact/implementation drift remains -> immediately call `openspec-archive-change <change-name>`.
- **OpenSpec Archive Is The Only Valid Terminal State:** For any task initiated via an OpenSpec skill or equivalent wrapper, control is returned to the user only when `openspec-archive-change <change-name>` has completed successfully, or when the Rule of 3 circuit breaker is triggered with a specific concrete blocker. Yielding after ff, propose, apply, verify, or a skipped/failed E2E gate is forbidden.
...
**OpenSpec override:** When the task was initiated via any OpenSpec skill or equivalent wrapper, conditions 1-3 are NOT sufficient on their own. The task is considered complete ONLY after `openspec-verify-change <change-name>` has produced zero `CRITICAL` and zero `WARNING`, any missing end-to-end coverage has been authored into the repository, the mandatory Playwright MCP or repo-native E2E gate has been actually executed and passed for the same change, and `openspec-archive-change <change-name>` has succeeded on the bound change. Completion of ff, propose, apply, verify, or E2E alone never satisfies the stop conditions. If verify or E2E fails, or required E2E coverage does not yet exist, the agent MUST return to `openspec-apply-change <change-name>` and continue the remediation loop. The only exception is the Rule of 3 circuit breaker with a concrete reported blocker.
