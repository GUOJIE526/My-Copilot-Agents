...
- **OpenSpec Chain Continuation:** If the user invokes `/opsx:ff` or `/opsx:propose` (or their hyphenated aliases), do not stop after proposal or artifact generation. Treat messages like "Run `/opsx:apply` when ready" as intermediate prompt output, not a stop condition. Capture the exact change name created or selected in that step and continue automatically on the same change.
- **OpenSpec Change Binding:** Reuse the exact same change name for every downstream OpenSpec step. Do not let later steps infer a different active change. Invoke implementation, verification, remediation, and archive against that bound change name explicitly so the flow does not stall on change-selection ambiguity.
- **OpenSpec Remediation Loop:** After `/opsx:ff` or `/opsx:propose`, immediately continue with `/opsx:apply <change-name>` and then `/opsx:verify <change-name>`. If verification reports any `CRITICAL` or `WARNING` findings, convert those findings into a targeted remediation checklist, fix them on the same change, then run `/opsx:apply <change-name>` again as needed and re-run `/opsx:verify <change-name>`. Repeat until verification has no `CRITICAL` findings and no `WARNING` findings, or until the Rule of 3 circuit breaker is reached.
- **OpenSpec Archive Gate:** Only run `/opsx:archive <change-name>` after all implementation tasks for that change are complete, verification returns no `CRITICAL` findings and no `WARNING` findings, and there is no remaining artifact or implementation drift. Never ask the user to manually issue the next OpenSpec command unless a real blocker prevents automatic continuation.
